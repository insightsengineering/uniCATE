...
estimate_univariate_cates <- function(
  ...
  # aggregate the beta coefficient vecotrs into a tibble
...
hold_out_calculation <- function(
  ...
  # for each biomarker
  Y_diff <- valid_data$Y_diff
  valid_data <- valid_data %>% dplyr::select(-Y_diff)
  num_obs <- nrow(valid_data)
  coefs_and_ic_ls <- valid_data %>%
    purrr::map(
      function(bio) {

        # compute the variance of the biomarker
        var_bio <- (num_obs - 1) / num_obs * stats::var(bio)

        # compute the linear regression without intercept for each biomarker
        bio_coef <- as.numeric(stats::coef(stats::lm(Y_diff ~ 0 + bio)))

        # compute the empirical IC of each observation
        inf_curves <- (1 / var_bio) * (Y_diff - bio_coef * bio) * bio

        # return the beta coefficients and the influnce curves
        return(list(
          "bio_coef" = bio_coef,
  ...
  # extract the table of influcnce curves
  ic_df <- lapply(seq_len(
    length(coefs_and_ic_ls)),
    function(idx) coefs_and_ic_ls[[idx]]$inf_curves
  )
  names(ic_df) <- biomarkers
  ic_df <- ic_df %>% dplyr::bind_cols()
