...
would be virtually identical to that given in the previous subsection.
...
We again perform an RCT where the outcome of interest is a potentially
right-censored time-to-event outcome. We consider $n$ i.i.d. copies of the
full-data random vector $X = (W, A, T^{(0)}, C^{(0)}, T^{(1)}, C^{(1)})$ 
distributed according to $P_X$ where $W$ and $A$ are defined as in the
continuous outcome case. $T^{(0)}$ and $T^{(1)}$ correspond to the failure times
under control and treatment conditions, and $C^{(0)}$ and $C^{(1)}$ correspond
to the censoring times under control and treatment conditions. As before, the
full-data random vectors are never observed; instead, we observe $n$ i.i.d.
copies of $O = (W, A, \Delta, \tilde{T})$ distributed according to $P_0$. Here,
$\Delta$ is a censoring indicator and
...
between the contrast of survival outcomes at time $t_0$ and each biomarker,
consisting of the simple linear regression coefficients of this contrast
...
As with the other variable importance metrics, this parameter can be
...
censoring mechanism is consistently estimated by the our procedure, the
...
# assemble the failure and censoring times into a tibble
status_df <- tibble(
...
mechanism and a failure mechanism that is a function of the interaction between
the treatment assignment $A$ and the biomarker $B1$. These failure and censoring
events occur at one of 9 discrete relative follow-up times for all 500 study
participants. Take special notice of the `censor` and `failure` columns that
describe the relative time measurement in `time`: the former is a binary
indicator of censoring, and the later of failure. Finally, note that
...
at time $t_0=6$ by setting the `time_cutoff` argument to `6`.
