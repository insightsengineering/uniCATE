test_that("sunicate() identifies the effect modified in a simulated example",{
  ...
  cond_surv_hazard <- function(t, treat, cov, biom1, biom2) {
    (t < 9) / (1 + exp(-(-1 - 0.2*cov - 0.3*biom1 + 10*treat*biom2))) +
      (t == 9)
  }
  cond_cens_hazard <- function(t, treat, cov, biom1, biom2) 0.15
  ...
  failure_time <- sapply(
    ...
    function(obs) {
      failure_time <- NA
      for (t in 1:9) {
        prob <- cond_surv_hazard(
          t, treat[obs], cov[obs], biom1[obs], biom2[obs]
        )
  ...
  # make sure that only biomarker 2 is a significant effect modifier
  biom1_pval <- res_tbl %>%
